...
class CustomGym:
    ...
    def set_state(self, state):
        self.env.unwrapped.init_qpos = state[:2]
        self.env.unwrapped.init_qpos = state[2:]
        return self.env
...
# class ExtendedCartPoleEnv(InvertedPendulumEnv):
#     def reset_model(self, state):
#         qpos = self.init_qpos + state[:2] + self.np_random.uniform(
#             size=self.model.nq, low=-0.01, high=0.01
#         )
#         qvel = self.init_qvel + state[2:] + self.np_random.uniform(
